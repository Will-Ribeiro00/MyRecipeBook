...
namespace WebApi.Test
{
    public class MyRecipeBookClassFixture : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private static void AddListToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, IList list)
        {
            var itemType = list.GetType().GetGenericArguments().Single();

            if (itemType.IsClass && itemType != typeof(string))
            {
                AddClassListToToMultipartContent(multipartContent, propertyName, list);
            }
            else
            {
        ...
        private static void AddClassListToToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, IList list)
        {
            var index = 0;
