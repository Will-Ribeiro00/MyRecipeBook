...
namespace WebApi.Test.Recipe.Update
{
    public class UpdateRecipeInvalidTokenTest : MyRecipeBookClassFixture
    {
        ...
        private readonly string _recipeId = "ABCABC";
        ...
        {
            // Arrange and Act
            var response = await DoPut(method: $"{METHOD}/{_recipeId}", request: _request, token: "tokenInvalido", culture: culture);
        ...
        {
            // Arrange and Act
            var response = await DoPut(method: $"{METHOD}/{_recipeId}", request: _request, token: string.Empty, culture: culture);
        ...
        public async Task ErrorTokenWithUserNotFound(string culture)
        {
            ...
            // Act
            var response = await DoPut(method: $"{METHOD}/{_recipeId}", request: _request, token: token, culture: culture);
