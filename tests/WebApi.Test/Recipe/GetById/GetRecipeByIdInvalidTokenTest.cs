...
namespace WebApi.Test.Recipe.GetById
{
    public class GetRecipeByIdInvalidTokenTest : MyRecipeBookClassFixture
    {
        ...
        public async Task ErrorTokenInvalid(string culture)
        {
            ...
            var response = await DoGet(method: $"{METHOD}/{recipeId}", token: "tokenInvalido", culture: culture);
        ...
        public async Task ErrorWithoutToken(string culture)
        {
            // Arrange and Act
            var recipeId = IdEncrypterBuilder.Build().Encode(1);
            var response = await DoGet(method: $"{METHOD}/{recipeId}", token: string.Empty, culture: culture);
        ...
        public async Task ErrorTokenWithUserNotFound(string culture)
        {
            ...
            // Act
            var response = await DoGet(method: $"{METHOD}/{recipeId}", token: token, culture: culture);
