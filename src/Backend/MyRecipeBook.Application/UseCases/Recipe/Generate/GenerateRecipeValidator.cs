...
namespace MyRecipeBook.Application.UseCases.Recipe.Generate
{
    public class GenerateRecipeValidator : AbstractValidator<RequestGenerateRecipeJson>
    {
        public GenerateRecipeValidator()
        {
            ...
            RuleFor(request => request.Ingredients.Count).InclusiveBetween(1, maximumNumberIngredients)
                .WithMessage(ResourceMessagesExceptions.INVALID_NUMBER_INGREDIENTS);

            RuleFor(request => request.Ingredients).Must(ingredients => ingredients.Count == ingredients.Distinct().Count())
                .WithMessage(ResourceMessagesExceptions.DUPLICATED_INGREDIENTS_IN_LIST);
